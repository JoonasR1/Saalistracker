...
function Stats(props) {
    ...
    const piecolors = randomColor({count: piedata.length, seed: "saalis tracker"});

    return (
        <div className={styles.stats}>
            <h2>Saalis stats</h2>
            <h3>Saaliit aikajanalla</h3>
            ...
            <LineChart data={linedata} margin={{top: 20, left: 20, right: 20, bottom: 10}} >
              ...
              <XAxis type="number" 
                ...
                />  
              <YAxis>
                  <Label className={styles.stats} value="Paino" 
                          position="left"
                          angle={-90}
                          style={{textAnchor:"middle" }} />
              </YAxis>  
              <Line dataKey="amount" name="paino" unit="KG" />
              <Tooltip labelFormatter={value => new Date(value).toLocaleDateString("fi-FI")} />  
            </LineChart>
            </ResponsiveContainer>

            <h3>Saaliit tyypettäin</h3>
